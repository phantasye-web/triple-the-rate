...
public class DonatorMainTab {
	...
	public static boolean isDonatorInterfaceMainTabButton(Player player, int buttonId) {
		...
		switch (buttonId) {
			// Purchase tokens
			case 129022:
				player.getPA().openWebsite("www.Runecessor.com/store", false);

				//player.getPA().sendMessage("click 'Store' on client links above to donate.");
				return true;
			...
			case 129047:
				new java.lang.Thread() {
					public void run() {
						try {
							com.everythingrs.donate.Donation[] donations = com.everythingrs.donate.Donation.donations("OIX8fWyMqG4Pw3TstHMBKW1XQDDYl9UI09BtW6XRa3Q8kd5cE9eAh03AtIzXg4sK4fVW4nfW", 
							...
							for (com.everythingrs.donate.Donation donate : donations) {
								ItemAssistant.addItem(player, donate.product_id, donate.product_amount);
							}
			...
			case 129052:
				player.getPA().sendMessage("Wish to get a good offer on donate ? Pm Michael");
				return true;
	...
	public static void updateAccountOffer(Player player, boolean action) {
		...
		if (hide) {
			...
			if (action) {
				...
				player.getPA().sendFrame126("Want to donate with osrs Pm Michael.", 33060);
				player.getPA().sendFrame126("we accept it osrs gold as payment", 33061);
