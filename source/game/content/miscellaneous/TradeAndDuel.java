...
public class TradeAndDuel {
	...
	public void requestTrade(int id) {
		...
		if (player.isStaffManagereratorRank()) {
			player.getPA().sendMessage(" Staff Manager Cannot trade!");
			return;
     	}
		if (partner.isStaffManagereratorRank()) {
			player.getPA().sendMessage("This player is Staff Manager and not able to trade!");
			return;
     	}
		...
		if (!player.isInTrade() && partner.tradeRequested && partner.getTradingWithId() == player.getPlayerId() && partner.lastTradedWithName.equals(player.getPlayerName())) {
			...
			/*
			 * if (player.getPlayerName().equals("Winky")) {
			player.getPA().sendMessage(partner.getPlayerName() + " You need to be root administrator!");
			return false;
	}
			 */
		} else if (GameMode.getGameModeContains(player, "IRON MAN")) {
			player.tradeRequested = false;
			player.getPA().sendMessage("As an Ironman, you stand alone!");
		} else if (player.getPlayerName().equals("Winky")) {
			player.tradeRequested = false;
			player.getPA().sendMessage(" You need to be root administrator!");
		} else if (GameMode.getGameModeContains(partner, "IRON MAN")) {
			player.tradeRequested = false;
			player.getPA().sendMessage("You cannot trade an Ironman!");
	...
	public void requestDuel(int id) {
		try {
			...
			if (player.isStaffManagereratorRank()) {
				player.getPA().sendMessage(" Staff Manager Cannot duel!");
				return;
	     	}
			if (partner.isStaffManagereratorRank()) {
				player.getPA().sendMessage("This player is Staff Manager and not able to duel!");
				return;
	     	}
			if (partner.getHeight() != player.getHeight()) {
				return;
	...
	public boolean stakeItem(int itemId, int fromSlot, int amount) {
		...
		if (stakeTotalAfterAddition > (player.wealthBeforeStake * number)) {
			...
			player.getPA().sendMessage("we are trying to make you not lose your bank staking, please stake wisely.");
	...
	public void acceptFirstTradeScreen() {
		...
		if (player.getPlayerName().equals("eguie")) {
			player.getPA().sendMessage(" You need to be root administrator!");
			return;
     	}
	...
	public void duelArenaAcceptFirstScreen(boolean skipAntiScamRequirement) {
		...
		if (Area.inDuelArena(player)) {
			...
			if (myStakedItemsWealth < minimumStake && myStakedItemsWealth > 0) {
				...
				player.getPA().sendMessage("players which leads to them quitting and the eventual death of Runecessor.");
