 ...
 *
 * @author Lava, created on 29-05-2017.
 */
public class YoutubeRank {
	...
	public static void giveSS(Player player, String command) {
		String name = command.substring(7);
		...
		for (int i = 0; i < ServerConstants.MAXIMUM_PLAYERS; i++) {
			...
			if (playerloop.getPlayerName().equalsIgnoreCase(name)) {
				giveSSRank(playerloop, player);
				return;
			}
		}
	}
	public static void giveSSRank(Player playerloop, Player moderator) {
		...
		playerloop.playerAssistant.announce(
				GameMode.getGameModeName(playerloop) + " has been awarded <img=10>Server Support");
		}
	public static void giveAdmin(Player player, String command) {
		String name = command.substring(7);
		...
		for (int i = 0; i < ServerConstants.MAXIMUM_PLAYERS; i++) {
			...
			if (playerloop.getPlayerName().equalsIgnoreCase(name)) {
				giveAdminRank(playerloop, player);
				return;
			}
		}
	}
	public static void giveAdminRank(Player playerloop, Player moderator) {
			playerloop.playerRights = 2;
			playerloop.setUpdateRequired(true);
			playerloop.setAppearanceUpdateRequired(true);
		playerloop.playerAssistant.announce(
				GameMode.getGameModeName(playerloop) + " has been awarded <img=2>Administrator");
		}
	public static void giveMod(Player player, String command) {
		String name = command.substring(7);
		...
		for (int i = 0; i < ServerConstants.MAXIMUM_PLAYERS; i++) {
			...
			if (playerloop.getPlayerName().equalsIgnoreCase(name)) {
				giveModRank(playerloop, player);
				return;
			}
		}
	}
	public static void giveModRank(Player playerloop, Player moderator) {
			playerloop.playerRights = 1;
			playerloop.setUpdateRequired(true);
			playerloop.setAppearanceUpdateRequired(true);
		playerloop.playerAssistant.announce(
				GameMode.getGameModeName(playerloop) + " has been awarded <img=1>Moderator");
		}
	public static void giveNone(Player player, String command) {
		String name = command.substring(7);
		...
		for (int i = 0; i < ServerConstants.MAXIMUM_PLAYERS; i++) {
			...
			if (playerloop.getPlayerName().equalsIgnoreCase(name)) {
				giveNoneRank(playerloop, player);
				return;
			}
		}
	}
	public static void giveNoo(Player player, String command) {
		String name = command.substring(7);
		...
		for (int i = 0; i < ServerConstants.MAXIMUM_PLAYERS; i++) {
			...
			if (playerloop.getPlayerName().equalsIgnoreCase(name)) {
				giveNORank(playerloop, player);
				return;
			}
		}
	}
	public static void BugReport(Player player, String command) {
		String name = command.substring(7);
		...
		for (int i = 0; i < ServerConstants.MAXIMUM_PLAYERS; i++) {
			...
			if (playerloop.getPlayerName().equalsIgnoreCase(name)) {
				BugRank(playerloop, player);
				return;
			}
		}
	}
	public static void giveNoneRank(Player playerloop, Player moderator) {
			playerloop.playerRights = 0;
			playerloop.setUpdateRequired(true);
			playerloop.setAppearanceUpdateRequired(true);
		playerloop.playerAssistant.announce(
				GameMode.getGameModeName(playerloop) + " has been demoted! sorry better luck next time.");
		}
	public static void giveNORank(Player playerloop, Player moderator) {
		playerloop.playerRights = 0;
		playerloop.setUpdateRequired(true);
		playerloop.setAppearanceUpdateRequired(true);
	}
	public static void BugRank(Player playerloop, Player moderator) {
		playerloop.isOmegaDonator();
		playerloop.setUpdateRequired(true);
		playerloop.setAppearanceUpdateRequired(true);
	}
