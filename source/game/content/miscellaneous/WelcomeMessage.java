 ...
 * Welcome message.
 *
 * @author Lava, created on 17-02-2016.
 */
	...
	public static void sendWelcomeMessage(Player player) {
		...
		if (StaffManagement.signUpThreadId >= 0 && StaffManagement.hoursPlayTimeRequired >= 0) {
			...
			if (player.isStaffManagereratorRank()) {
				player.getPA().sendMessage(
						"<img=31>Staff Manager: Sign ups is set to thread " + StaffManagement.signUpThreadId + " and hours required " + StaffManagement.hoursPlayTimeRequired + " and");
				player.getPA().sendMessage("last 7 days active hours to " + StaffManagement.hoursActivePlayedTimeInTheLastWeek + ".");
			}
		} else {
			if (player.isStaffManagereratorRank()) {
				player.getPA().sendMessage("<img=31>Staff Manager: Sign ups is currently off.");
			}
		}
	}
