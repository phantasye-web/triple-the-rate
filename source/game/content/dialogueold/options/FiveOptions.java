...
public class FiveOptions {
		...
		switch (player.getDialogueAction()) {
			...
			case 199:
				...
				if (!(player.getTotalLevel() >= 2178)) {
					player.getPA().sendMessage("You need 2178 total level to buy this cape.");
					break;
				}
	...
	public static void secondOption(Player player) {
		...
		case 199:
					...
					player.playerAssistant.sendMessage("Coming Soon!");
	...
	public static void fourthOption(Player player) {
		...
		switch (player.getDialogueAction()) {
			...
			//player.getPA().sendMessage("click 'Store' on client links above to donate.");
				player.getPA().openWebsite("https://runecessor.com/forum/index.php?/forum/7-donation-information/", false);
				player.getPA().closeInterfaces(true);
				break;
