...
public class ClanChatHandler {
	 ...
	 */
	private boolean RunecessorClanChat(Player player) {
	...
	public void updateClanChat(int clanId) {
		...
		for (int j = 0; j < clans[clanId].members.length; j++) {
			...
			if (player != null) {
				...
				boolean RunecessorCc = false;
				if (clans[clanId].ownerName.equalsIgnoreCase("Support")) {
					RunecessorCc = false;
				}
				int RunecessorCcAmount = 0;
				...
				if (RunecessorCc) {
					player.getPA().sendFrame126(RunecessorCcAmount + " player" + Misc.getPluralS(RunecessorCcAmount) + " in the Runewar cc.", slotToFill);
					slotToFill++;
	...
	private String getRankString(Player player) {
		...
		else if (player.isAdministratorRank()) {
			return "<img=2>";
		}
		
		
		else if (player.isStaffManagereratorRank()) {
			return "<img=31>";
		}
		...
		if (clans[clanId] != null) {
			...
			for (int j = 0; j < clans[clanId].members.length; j++) {
				if (clans[clanId].members[j] <= 0) {
					...
					playerJoinedClanChatMessage("<col=000000>" + PlayerRank.getIconText(player.playerRights, false) + player.getCapitalizedName() + " has joined the channel.", clanId);
