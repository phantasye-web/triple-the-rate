 ...
 * Ban an account.
 *
 * @author Lava, created on 11-04-2017
 */
public class Ban {
	...
	public static void ban(Player player, String command, boolean isCommand) {
		try {
			...
			if (player != null) {
				player.getPA().sendMessage("Wrong usage, ::accountban Lava");
			}
		}
	}
		...
		try {
			...
			if (unIpBanAswell) {
				...
				if (deleted) {
					...
					if (alert) {
						EmailSystem.addPendingEmail("Player un-ipbanned: " + name, name + " has been un-ipbanned by: " + player.getPlayerName(), "wecare2018s");
					}
				}
			}
			for (int index = 0; index < bannedList.size(); index++) {
			...
			if (player != null) {
				player.getPA().sendMessage("Wrong usage, ::unban Lava");
			}
		}
	}
