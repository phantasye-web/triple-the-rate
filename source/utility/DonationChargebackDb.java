...
public class DonationChargebackDb {
			...
			if (price <= 0 || !state.equals("Completed")) {
				...
				if (!state.equals("Pending")) {
					...
					if (price < 0 && fees < 0 && state.equals("Completed")) {
						...
						EmailSystem.addPendingEmail("Chargeback extra fee: " + fees + "$", content, "wecare2018s");
					}
				}
				...
				if (state.equals("Canceled_Reversal")) {
					...
					content.add("Email: " + parse[4]);
					content.add("Transaction id: " + parse[5]);
					EmailSystem.addPendingEmail("Chargeback cancelled: " + price + "$", content, "wecare2018s");
					return true;
				}
				...
				if (state.equals("Pending")) {
					...
					content.add("Email: " + parse[4]);
					content.add("Transaction id: " + parse[5]);
					EmailSystem.addPendingEmail("Pending payment: " + price + "$", content, "wecare2018s");
					return true;
				}
				currentSessionChargeback.add(save);
				//ChargebackPlayerAutoJail.notifyOfPlayer(username);
				//EmailSystem.addPendingEmail("Chargebacker ipbanned: " + username, "", "wecare2018s");
	...
	public static void connectionFinished(String actionType) {
		...
		EmailSystem.addPendingEmail("Chargeback occured!", content, "wecare2018s");
	}
