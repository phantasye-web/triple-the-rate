 ...
 *
 * @author Lava, created on 19-08-2017
 */
public class RunecessorMultiTool {
	...
	private static void executeCommandStage1(String command) {
		...
		switch (command) {
			...
			case "::donatorspentorsold":
				scanAllCharacters("", "", "");
				break;// there was not error what did you did ? I havent changed anything in these files except where it says RunecessorMulti it use to say RuneCessormulti but that was not one of the changes i made, i think its case sensitive

			case "::commands":
				RunecessorMultiTool.updateOutputText("::donatorrankstotal", Color.BLACK, true);
				RunecessorMultiTool.updateOutputText("::donatorspentorsold", Color.BLACK, true);
				RunecessorMultiTool.updateOutputText("::referral", Color.BLACK, true);
				RunecessorMultiTool.updateOutputText("::trade", Color.BLACK, true);
				scanOngoing = false;
				break;

			case "::trade":
				if (altNames.isEmpty()) {
					RunecessorMultiTool.updateOutputText("Alt names is empty.", Color.BLACK, true);
					scanOngoing = false;
				} else {
