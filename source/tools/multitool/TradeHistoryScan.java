 ...
 *
 * @author Lava, created on 28-09-2017
 */
public class TradeHistoryScan {
	...
	public static void tradeHistoryScan() {
		...
		RunecessorMultiTool.scanOngoing = false;
		...
		for (int index = 0; index < sort.size(); index++) {
			...
			for (int i = 0; i < data.size(); i++) {
				...
				for (int h = 0; h < RunecessorMultiTool.altNames.size(); h++) {
					if (!line.toLowerCase().contains("[" + RunecessorMultiTool.altNames.get(h) + "]")) {
						continue;
					}
					...
					for (int z = 0; z < split.length; z++) {
						...
						for (int a = 0; a < RunecessorMultiTool.altNames.size(); a++) {
							String playerName = RunecessorMultiTool.altNames.get(a);
							playerName = "[" + playerName + "]";
							if (text.contains(playerName)) {
								RunecessorMultiTool.updateOutputText(text + " ", Color.RED, z == split.length - 1 ? true : false);
								applied = true;
								break;
							}
						}
						if (!applied) {
							RunecessorMultiTool.updateOutputText(text + " ", Color.BLACK, z == split.length - 1 ? true : false);
						}
					}
					break;
				}
		...
		RunecessorMultiTool.scrollPane.getVerticalScrollBar().setValue(0);
	}
